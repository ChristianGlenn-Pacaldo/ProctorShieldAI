...
export default function StudentDashboardContent() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Join Quiz Box */}
      <div className="p-5 rounded-xl bg-blue-600/5 border border-blue-500/20 shadow-xs">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h3 className="font-bold text-blue-600 dark:text-blue-400 mb-1 font-[family-name:var(--font-display)]">Join an Quiz</h3>
            <p className="text-xs text-[var(--muted)]">
              Enter the access code provided by your instructor to start your proctored session.
            </p>
          </div>
          <div className="flex gap-2.5">
            <input
              ...
              placeholder="PS-8821"
              className="w-36 px-4 py-2.5 text-center font-bold tracking-widest text-sm rounded-xl bg-[var(--surface)] border border-[var(--border)] text-[var(--ink)] placeholder:text-[var(--muted2)] focus:outline-none focus:border-blue-500"
            />
            <button
              onClick={handleJoinQuiz}
              disabled={isLoading}
              className="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-all shadow-xs disabled:opacity-50"
            >
