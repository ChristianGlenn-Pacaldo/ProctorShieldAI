...
export default function PlaygroundContent({
  ...
  const handleLaunchArena = async () => {
    ...
    setLaunchError("");
    try {
      const response = await fetch(`/api/arena/${selectedQuizId}`, {
        method: "POST",
      ...
      if (!response.ok || !data?.success) {
        setLaunchError(data?.error || "Unable to create the Arena lobby.");
        return;
      }
      ...
      setLaunchError("Unable to create the Arena lobby. Please try again.");
    } finally {
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
          ...
          <div className="flex flex-col gap-3 w-full sm:w-auto shrink-0">
            ...
            <div className="text-xs text-center text-slate-400">
              Select an unfinished Power Arena quiz to start
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {quizzes.length === 0 ? (
          <div className="p-8 rounded-2xl bg-slate-900/60 border border-slate-800 text-center space-y-3">
            ...
            <div className="text-base font-bold text-white">No Launchable Power Arena Quiz</div>
            <p className="text-sm text-slate-400 max-w-md mx-auto">
              Create a new Power Arena quiz first. Completed Arena quizzes remain review-only and cannot host another match.
            </p>
            <Link
              href="/dashboard/teacher/quizzes?create=true&mode=arena"
      ...
      {isModalOpen && (
        ...
        >
          ...
          >
            ...
            {/* Modal Actions */}
            {launchError && (
              <div className="rounded-xl border border-rose-500/40 bg-rose-500/10 px-4 py-3 text-xs font-bold text-rose-200">
                {launchError}
              </div>
            )}
            <div className="pt-4 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                ...
                <button
                  type="button"
                  disabled={!selectedQuizId || isLaunching}
                ...
                >
                  {isLaunching ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5 fill-slate-950" />}
                  {isLaunching ? "Creating Lobby..." : "Create Arena Lobby"}
                </button>
              </div>
            </div>
          </div>
